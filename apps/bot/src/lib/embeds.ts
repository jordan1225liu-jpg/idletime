...
export function buildWelcomeEmbed(characterName: string): EmbedBuilder {
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        '你來到了一座中世紀小鎮,生活緩慢但豐富。現在你可以:',
        '',
        '🌾 `/farm` 種田 ・ 🎣 `/fish` 釣魚 —— 累積資源、練技能',
        '🎒 `/inventory` 看背包 ・ 💰 `/sell` 賣作物換金幣',
        '🧪 `/brew` 用作物煉回血藥',
        '🏪 `/shop` 買武器護甲 ・ ⚔️ `/equipment` 裝備提升戰力',
        '🏹 `/hunt` 1-3 人組隊打怪,賺主等級與金幣',
        '🤝 `/visit @朋友` 拜訪,雙方各得 +20⚡',
        '🏆 `/leaderboard` 看全球 / 伺服器排行榜',
        '',
        '**新手三步驟:**',
        '① `/me` 看你的角色',
        '② `/story` 跟著鎮長踏上重建灰燼谷的旅程 📜',
        '③ `/farm` 種第一批作物(選作物一鍵種滿)',
        '',
        '_更多生活技能(採集、烹飪、打鐵)與公會系統開發中 🚧_',
      ].join('\n'),
    );
}
